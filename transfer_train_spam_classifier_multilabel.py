...
f = open('historical_raw_data_multilabel.npy', 'wb')
np.save(f, raw_data)
f = open('historical_header_multilabel.npy', 'wb')
np.save(f, header)
'''
# load data 
raw_data = np.load('historical_raw_data_mulitlabel.npy', allow_pickle=True)
header = np.load('historical_header_multilabel.npy', allow_pickle=True)
'''
# load checkpoint (encoder with categories, weights)
modelName = 'text-class.12-0.12.pkl'
